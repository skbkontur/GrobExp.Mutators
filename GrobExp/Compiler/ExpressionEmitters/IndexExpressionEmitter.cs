    ...
    internal class IndexExpressionEmitter : ExpressionEmitter<IndexExpression>
    {
        protected override bool Emit(IndexExpression node, EmittingContext context, GroboIL.Label returnDefaultValueLabel, ResultType whatReturn, bool extend, out Type resultType)
        {
            ...
            if(node.Indexer != null)
            {
                ...
                if(node.Object.Type.IsDictionary() && extend)
                {
                    ...
                    if(extend)
                    {
                        ...
                        using(var value = context.DeclareLocal(valueType))
                        {
                            ...
                            if(valueType.IsValueType)
                            {
                                ...
                                context.Il.Ldloc(value); // stack: [dict, key, default(valueType)]
                            }
                            else
                            {
                                if(!valueType.IsArray)
                                    context.Il.Newobj(constructor); // stack: [dict, key, new valueType()]
                                else
                                {
                                    context.Il.Ldc_I4(0);
                                    context.Il.Newarr(valueType.GetElementType());  // stack: [dict, key, new valueType[0]]
                                }
                            }
                            ...
                            context.Il.Call(setter, node.Object.Type); // dict.set_Item(key, default(valueType)); stack: [dict]
