...
namespace GrobExp.Mutators
{
    public class ModelConfigurationNode
    {
        ...
        public LambdaExpression BuildTreeValidator(IPathFormatter pathFormatter)
        {
            ...
            root.BuildValidator(pathFormatter, this == Root ? null : this, aliases, result, treeRootType, priority, validationResults);
...
*/
        ...
        private class ZzzNode
        {
            ...
            public void BuildValidator(IPathFormatter pathFormatter, ModelConfigurationNode root, List<KeyValuePair<Expression, Expression>> aliases, ParameterExpression result, Type treeRootType, ParameterExpression priority, List<Expression> validationResults)
            {
                foreach(var pair in mutators)
                    BuildNodeValidator(pathFormatter, pair.Key, pair.Value, root, aliases, result, treeRootType, priority, validationResults);
                foreach(var pair in children)
                {
                    ...
                    child.BuildValidator(pathFormatter, root, aliases, result, treeRootType, priority, childValidationResults);
                    aliases.RemoveAt(aliases.Count - 1);
                    aliases.RemoveAt(aliases.Count - 1);
            ...
            private static void BuildNodeValidator(IPathFormatter pathFormatter, Expression path, List<MutatorConfiguration> mutators, ModelConfigurationNode root, List<KeyValuePair<Expression, Expression>> aliases, ParameterExpression result, Type treeRootType, ParameterExpression priority, List<Expression> validationResults)
            {
...
//                return Expression.Block(typeof(object[]), new[] { indexes }, indexesInit, /*writeExp, writeIndexes,*/ Expression.NewArrayInit(typeof(object), result));
                var resolvedArrayIndexes = chains.Select(expression => eachesResolver.Visit(expression).ResolveArrayIndexes()).ToArray();
                ...
                //Expression cutChains = Expression.NewArrayInit(typeof(object[]), );
